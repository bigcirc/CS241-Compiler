  ...
  try{
	...
	// OPERATOR
	  ...
	  // 2 inpur operator
	      ...
	      if( it2+1 == it->end() || it2+2 == it->end() || it2+3 == it->end() ){
		cerr << "ERROR: not enough entries, expected 5 with commas" << endl;
		error = true;
		break;
	      } else if( it2+4 == it->end() ){
		if( (*(*(it2+1))).getKind() == ASM::REGISTER && 
		  ...
		  if( op == "mult" ){ instr = (0 << 26) | (reg1 << 21) | (reg2 << 16) | 0x00000018; }
		  else if( op == "multu" ){ instr = (0 << 26) | (reg1 << 21) | (reg2 << 16) | 0x00000019; } 
		  else if( op == "div" ){ instr = (0 << 26) | (reg1 << 21) | (reg2 << 16) | 0x0000001a; } 
		  else if( op == "divu" ){ instr = (0 << 26) | (reg1 << 21) | (reg2 << 16) | 0x0000001b; } 
		  printNum( instr );
		
		} else { //Not a register
		  cerr << "ERROR: not valid input, expected REG, REG" << endl;
		  error = true;
		  break;
		}

		it2+=3; //skip the tokens after ID
		PC+=4;
	      } else {
		cerr << "ERROR: more stuff after ID, expected 1 entry" << endl;
		error = true;
	  ...
	  //Special case for lw or sw
	  } else if( op == "lw" || op == "sw" ){
	 
	    if( it2+1 == it->end() || it2+2 == it->end() || it2+3 == it->end() || it2+4 == it->end() || it2+5 == it->end() || it2+6 == it->end() ){
	      cerr << "ERROR: not enough entries, expected 5 with commas" << endl;
	      error = true;
	      break;
	    } else if( it2+7 == it->end() ){
	      if( (*(*(it2+1))).getKind() == ASM::REGISTER && 
		  (*(*(it2+2))).getKind() == ASM::COMMA && 
		...
		unsigned int reg1 = (*(*(it2+1))).toInt();
		unsigned int reg2 = (*(*(it2+5))).toInt();
		int offset = (*(*(it2+3))).toInt();
		unsigned int instr;
		     ...
		     offset < 32768)
		    ||
		    ((*(*(it2+3))).getKind() == ASM::HEXINT &&  
		     offset < 0x10000) ){
		    
		    if( op == "lw" ){ instr = (35 << 26) | (reg2 << 21) | (reg1 << 16) | offset & 0xffff; }
		    else if( op == "sw" ) { instr = (43 << 26) | (reg2 << 21) | (reg1 << 16) | offset & 0xffff; }
		    printNum( instr ); 
		...
		cerr << "ERROR: invalid input, expected REG, INT(REG)" << endl;
		error = true;
		break;
	      }
	      
	      it2+=6; //skip the register tokens after ID
	      PC+=4;
