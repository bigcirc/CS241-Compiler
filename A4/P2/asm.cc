  ...
  try{
	...
	// OPERATOR
	  ...
	  // 3 input operator  
	    ...
	    } else if( it2+6 == it->end() ){ //nothing after ID REG,REG,REG
	      if( (*(*(it2+1))).getKind() == ASM::REGISTER && (*(*(it2+2))).getKind() == ASM::COMMA && (*(*(it2+3))).getKind() == ASM::REGISTER && (*(*(it2+4))).getKind() == ASM::COMMA && (*(*(it2+5))).getKind() == ASM::REGISTER  ){
		...
		break;
	      ...
	      it2+=5; //skip the register token after ID
	    } else {
