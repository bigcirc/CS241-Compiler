...
void fillSymbolTable( vector< pair<string, int> > symbolTable, vector< vector<Token*> > tokLines, bool error, int addressCount ) {
    // Iterate over the lines of tokens
...
int main(int argc, char* argv[]){
  ...
  try{
    ...
    fillSymbolTable( symbolTable, tokLines, error, addressCount ); //Pass 1
	...
	//DOTWORD
	  ...
	  if( it2+1 == it->end() ) { //nothing after .word
	    cerr << "ERROR: no word after .word" << endl;
	    error = true;
	    break;
	  
	  } else if( it2+2 == it->end() ){ //no extra stuff after .word x
	    
	    if( (*(*(it2+1))).getKind() == ASM::INT || (*(*(it2+1))).getKind() == ASM::HEXINT ){
	      
	      printNum( (*(*(it2+1))).toInt());
	    	      
	    } else { //not an integer, check for matching label
	      
	      if( !checkLabel( (*(*(it2+1))).getLexeme(), symbolTable)){
		cerr << "ERROR: label not declared" << endl;
		error = true;
		break;
	      }
	  ...
	  ++it2; //skip operand that follows .word
	  ++addressCount;
